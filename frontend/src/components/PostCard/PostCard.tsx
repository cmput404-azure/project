...
function PostCard({ post_obj, onCommentButtonClick, onClick }: PostCardProps) {
  ...
  const handleClickShare = async () => {
    ...
    if (post_obj.visibility === 1 || post_obj.visibility === 3) {
      for (const follower of followers) {
        const inboxResponse = await inbox.sendPostToInbox(
          follower.id,
          post_obj
        );
      }
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.headerText}>
          ...
          <span className={styles.postTime}>
            {new Date(post_obj.published).toLocaleString()}
          </span>
        </div>
        ...
        <div className={styles.cardContent}>
          ...
          {post_obj.contentType != ContentType.MARKDOWN &&
          post_obj.contentType != ContentType.PLAIN ? (
