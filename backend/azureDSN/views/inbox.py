...
class InboxView(APIView):
    ...
    def create_post(self, user_object, payload, request):
        try:
            ...
            return Response(
                {"message": "We have noticed other users about your post"},
                status=status.HTTP_200_OK,
            )
    ...
    def send_like_to_remote(self, payload, request, test=False):
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 201:
                return Response(
                    {"message": "Like sent to remote inbox."}, status=status.HTTP_200_OK
                )
    ...
    def create_like(self, user_object, payload, request):
        ...
        return Response(
            {"message": "Notice post's owner about your like successfully"},
            status=status.HTTP_200_OK,
        )
    ...
    def create_share(self, user_object, payload, author_serial):
        ...
        if User.objects.filter(uuid=author_serial).exists():
            ...
            if serializer.is_valid():
                ...
                return Response(
                    {"message": "Store share successfully"}, status=status.HTTP_200_OK
                )
            else:
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response(
                {"message": "Don't handle remote user"}, status=status.HTTP_200_OK
            )
